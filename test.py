...
# 条件2：主力次主力合约不能为同一张合约
        ...
        check2 = subOI['S_INFO_WINDCODE']==subOI['S_INFO_WINDCODE1']

# # 条件3：如果出现合约退市仍作为主力合约的情况，那么顺延到下一个主力合约
#   感觉不需要这部分,因为有交易数据的部分都不会退市
        delistdate = self.future_delistdate()
        delistdate['S_INFO_CODE']=pd.DataFrame([i[2:6] for i in delistdate['S_INFO_WINDCODE']])
        delistdate=delistdate.rename(columns = {'S_INFO_WINDCODE':'S_INFO_WINDCODEx'})
        subOI = pd.merge(delistdate[['S_INFO_WINDCODEx','S_INFO_CODE','S_INFO_LISTDATE','S_INFO_DELISTDATE']],subOI,how = 'right',on = ['S_INFO_CODE'])
        check3 = (subOI['S_DQ_CLOSE'].astype(str) == 'nan')
# =============================================================================
# 条件合并
        #输入满足条件1的换仓位
