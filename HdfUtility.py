    ...
    def hdfWrite(self,path,excode,symbol,startdate,enddate,indata,kind):
        ...
        if kind in ['00','01']:
            ...
            #如果该地址已存在已有数据，以下代码正常运行
                ...
                if temp_rule.TRADE_DT.iloc[-1] > startdate:
                    #如果[startdate,enddate]中已有对应于数据库中的数据
                    #找到数据重合位置,并提取该调整因子数值
                    temp_adj1 = indata.adj_factor.ix[indata.TRADE_DT == temp_rule.temp_rule.TRADE_DT.iloc[-1],:]
                    temp_adj2 = temp_rule[EXT_Out_AdjFactor].iloc[-1]
                    temp_adj = temp_adj2/temp_adj1
                elif temp_rule.TRADE_DT.iloc[-1] < startdate:
                    #时间差大于4时，则有可能数据出现截断，每次录数据尽可能从每月1日开始
                    temp_adj = temp_rule[EXT_Out_AdjFactor].iloc[-1]
                else:
                    print('写入数据与现有数据可能出现不连贯性')
                    temp_adj = 1
                adddata = indata.ix[(temp_rule.TRADE_DT>=temp_rule.TRADE_DT.iloc[-1] & temp_rule.TRADE_DT<enddate),:]
                #保证调整因子的连贯性
